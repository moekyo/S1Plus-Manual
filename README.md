...
- **高级帖子屏蔽**：
  ...
  - **楼层屏蔽**：可在帖子内屏蔽特定楼层的回复，无需屏蔽整个用户。在设置面板中可按帖子分组管理已屏蔽的楼层。
...
- **强大的设置中心与数据同步**：
  ...
  - **云端同步**：支持通过 GitHub Gist 实现全自动、跨设备的配置同步。请求层内置超时与重试机制，并支持大文件 Gist 自动补拉取。
  - **智能无感同步**：采用 V5 智能同步引擎，能精确区分阅读进度与核心数据的差异。当浏览帖子时，若检测到仅有当前帖子进度不同，会自动在后台完成同步，无需任何操作。
  - **并发与冲突防护**：内置跨标签页同步锁、冲突弹窗冷却和冲突后自动暂停机制，减少多标签并发导致的数据互相覆盖风险。
  - **数据安全与完整性**：同步过程采用 SHA-256 哈希值校验，确保数据在传输过程中的完整性，能有效防止云端数据损坏污染本地配置。
  - **同步诊断**：在设置面板三连击版本号即可打开隐藏诊断面板，查看最近同步尝试/成功/失败信息并一键复制，便于排障。
  - **冲突解决与手动控制**：提供强大的手动同步功能，当出现同步冲突时，会清晰展示本地与云端数据的时间戳，让用户自主选择保留哪个版本。同时新增**高级模式**，可在导航栏悬停直接选择“推送”或“拉取”。
  - **Token 维护增强**：支持 Sync Token 过期时间提醒（到期前预警）与 PAT 显示/隐藏，降低因凭证过期或输入错误导致的同步中断。
...
- **帖子屏蔽**：管理手动屏蔽的帖子列表、已屏蔽的楼层（按帖子分组），并配置关键字/正则屏蔽规则。
