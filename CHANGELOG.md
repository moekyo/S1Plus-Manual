...
- **设置跨标签刷新链路增强（Tampermonkey 兼容）**: 针对部分环境下 `s1p_settings` 变更监听不稳定的问题，新增独立信号键 `s1p_settings_refresh_signal`。每次 `saveSettings` 写入后同步发信号，并复用核心数据跨标签监听通道触发“从存储拉取设置快照 + 立即应用”流程，保证功能开关在其他标签页稳定收敛。
- **设置跨标签兜底同步**: 新增可见页轻量对齐机制（`visibilitychange`/`focus`/`pageshow` + 周期轮询），当事件监听缺失或误报时仍可检测并补偿设置漂移，避免 A 页关开关后 B 页残留旧状态。
