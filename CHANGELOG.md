...
- **[兼容性]** 修复了在 S1 NUX 主题的窄屏模式下，导航栏中的“手动同步”按钮因外边距计算错误而导致的背景断裂和对齐问题。
- **[UI]** 修复了行内操作菜单（如用户标记菜单）在屏幕左侧边缘附近显示时，可能因计算错误而被遮挡或定位不准确的问题。

### ✨ 新增与增强 (Features & Enhancements)

- **[集成] 新增 S1 NUX 安装推荐**: 为了提升用户的综合浏览体验，当脚本检测到用户未安装广受好评的 `S1 NUX` 论坛美化扩展时，会适时（每7天最多一次）弹出对话框进行推荐。用户可以在“设置”->“界面与个性化”中手动关闭此推荐。
- **[设置] 统一设置面板交互范式**: 将所有主要功能标签页（如通用、导航栏、用户屏蔽等）重构为由一个总开关控制的、可折叠的统一结构。这使得禁用或启用一整类功能更加直观，同时也让设置界面更加整洁有序。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **[设置] 为所有危险操作增加二次确认**: 为设置面板中的所有删除/恢复类操作（如取消屏蔽、删除规则、恢复默认等）增加了二次确认对话框，以防止用户误操作导致数据丢失。
- **[UI] 全新 V4 按钮系统**: 全面升级了脚本中所有按钮的视觉风格，采用更精致的阴影和悬停时的“上浮”动画效果，替代了原有的描边样式，提升了整体的现代感和交互体验。
- **[UI] 优化危险操作按钮交互**: 重新设计了所有危险操作按钮（如删除、恢复默认等）。现在它们默认显示为中性灰色，仅在鼠标悬停时变为红色，以减少视觉干扰并有效防止误操作。
- **[UI] 优化设置面板布局**: 调整了设置面板内的分组 (`group`) 与标题 (`title`) 样式，通过更清晰的视觉层级划分，提升了信息的可读性。

### 📦 重构 (Refactoring)

- **[CSS] 统一与简化按钮样式**: 对按钮相关的 CSS 进行了重构，移除了多个冗余的样式类，将所有按钮统一到 `.s1p-btn` 基类下进行管理，显著提高了代码的可维护性和一致性。

### ✨ 新增与增强 (Features & Enhancements)

- **[用户标记]** 为用户标记的删除操作添加了独立的行内二次确认菜单，取代了通用的确认方式，交互更流畅、稳定。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **[用户标记]** 重构了用户标记操作菜单的交互逻辑，触发方式由“点击”改为“鼠标悬停”，并优化了菜单与二次确认框的联动，彻底解决了此前菜单容易意外关闭和闪烁的问题。
- **[用户标记]** 调整了用户标记菜单及相关组件的样式（内边距、行高），使其在视觉上更对齐、更美观。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **[UI/UX]** 重构和优化了帖子列表的布局，使其在视觉上更整洁、更稳定。
  - **隐藏原生图标**: 隐藏了帖子列表左侧的原生状态图标（如“新窗”图标），并将该列宽度压缩为零，使布局更紧凑。
  - **统一操作列**: 为脚本注入的操作列（三点菜单）和表头占位符提供了统一、明确的宽度和样式，确保了在各种情况下（如不同主题、缩放比例）的对齐和显示一致性。
  - **简化样式**: 移除了一些过时或冗余的 CSS 规则，提高了代码的简洁性和可维护性。

### ✨ 新增与增强 (Features & Enhancements)

- **优化远程同步首次设置体验**:
  - 彻底重构了在设置面板中保存 Gist ID 和 PAT 后的逻辑。
  - **防止意外覆盖**: 保存设置时不再盲目触发自动同步，从根本上解决了首次配置时，本地的默认设置意外覆盖云端已有备份的严重问题。
  - **智能首次同步**: 保存凭据后，脚本现在会自动调用完整的“手动同步”流程。该流程会智能检测云端状态（如备份为空、存在冲突或有新数据），并弹出清晰的对话框引导用户完成首次数据推送或拉取，确保数据安全。
...
- **[Fix]** 移除了标签页内容区域多余的淡入动画，使切换更直接。
- **[Style]** 微调了同步按钮的旋转动画方向。

### 🐛 修复 (Fixed)

- 修复了收藏包含已屏蔽用户引用的楼层时，会将引用占位符一并存入收藏内容的问题。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- 优化了收藏夹的帖子来源链接样式，当标题过长时会自动截断并显示省略号，防止布局破坏，并增加了跳转图标提升视觉效果。

### ✨ Style

- **设置面板 UI 重构**:
  - **Tabs 样式升级**: 将设置面板的导航 Tabs 从传统下划线样式重构为现代化的 Pill (药丸) + 滑块样式，并增加了流畅的滑动动画和内容淡入效果，提升了视觉吸引力和交互体验。
  - **控件样式优化**:
    - 将悬浮控制按钮的把手图标从文字 `•••` 替换为更清晰的 SVG 矢量图标。
    - 微调了悬浮控制按钮把手的高度，使其更加紧凑。
- **动画性能提升**:
  - 优化了分段控件 (Segmented Control) 和新版 Tabs 的滑块动画，优先使用 `transform` 属性代替 `left`，以获得更流畅的动画性能。

### ✨ 新增与增强 (Features & Enhancements)

- **【UI】新增增强型悬浮控件**:
  - 新增一套现代化、可悬停展开的页面悬浮控件（包含返回顶部/底部、快速回复、返回列表），用于替代论坛原生控件。
  - 在“设置”->“界面与个性化”中增加了对应的开关，允许用户在 S1 Plus 新控件和论坛原生控件之间自由切换，设置即时生效。
...
- **【UI】精细调优“当前阅读位置”指示器垂直对齐**
  - 修正了指示器在不同操作系统下（尤其是 Windows）图标与文本存在的细微垂直偏移问题。通过引入非对称内边距和针对性的像素微调，确保了指示器内容在视觉上的完美居中，提升了跨平台显示的一致性
...
- **【主题兼容】新增 S1 NUX 主题兼容模式**
  ...
  - **开启时 (默认)**：脚本不再强制覆写滚动条和 UI 组件颜色，以完美兼容 S1 NUX 主题的外观。
  - **关闭时**：恢复 S1 Plus 经典的绿色滚动条和 UI 高亮颜色。
...
- **统一主题色**: 调整了脚本 UI 的主题色，使其与 S1 NUX 风格更加匹配。
