...
- **全屏蒙版模板边界收口**: 顶层设置面板、确认类弹窗（欢迎弹窗、S1 NUX 推荐、手动同步选择/冲突、Token 过期提醒、阅读记录详情等）和图片查看器统一走 `s1p-fullscreen-modal` 蒙版模板；设置面板内的 Token 日期配置弹窗改为挂载到 `.s1p-modal-content` 内部，由设置面板承载，不再额外叠加全局蒙版。
- **全屏蒙版动效时序优化**: 顶层设置面板、确认类弹窗、同步冲突提示、阅读记录详情和图片查看器统一改为“打开先蒙版后内容、关闭先内容后蒙版”的队列式时序，并让确认类弹窗进退场缩放终点一致。
- **确认弹窗本体材质收口**: 一级确认弹窗内容壳 `.s1p-confirm-content` 改为复用 `--s1p-floating-surface-*` 背景、阴影和磨砂滤镜；原 `--s1p-dialog-glass-*` 配置保留给设置大面板、Token 日期配置和手动同步弹窗内部信息块使用。
- **设置承载二级弹窗磨砂样式统一**: 抽出设置面板二级弹窗专用磨砂样式，Token 有效期、阅读记录详情、设置内确认/输入弹窗和设置内手动同步选择统一显式 opt-in；移除阅读记录表格斑马纹，并让深浅色文字跟随脚本主题色。
