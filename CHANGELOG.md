...
- **全局 toast 提示换行优化**: 将面板 toast 的两遍测量自适应逻辑扩展到全局 toast，用 `nowrap` + `max-content` 先测文本真实所需宽度，空间充足则保持单行、超出才换行；同步将全局 toast CSS 的废弃 `word-break: break-word` 改为 `overflow-wrap: break-word; word-break: normal`。
