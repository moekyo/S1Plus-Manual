...
- **前台探测补偿重试增强**: 前台自动检查云端更新命中远端变化后，若 follow-up sync 因已有同步任务或锁占用被跳过，会自动安排补偿重试，降低“明明云端已更新但偶发没自动拉取”的概率。
- **自动拉取刷新策略统一**: `pulled`、`force_pulled` 与阅读进度自动合并后的导入统一接入同一套刷新策略；普通自动拉取在线程页也会按规则刷新，减少“其实已拉到本地但页面看起来没变”的困惑。
...
- **阅读进度合并构建逻辑收敛**: 抽离统一的阅读进度合并 payload 构建 helper，自动合并与手动智能合并复用同一套数据拼装逻辑，减少重复分支并降低后续维护成本。
- **同步成功后的刷新入口统一**: 新增面向同步结果的刷新策略接入 helper，收敛启动同步、每次加载检查、后台同步、前台 follow-up sync 的成功后处理逻辑。
- **前台补偿重试状态机收敛**: 将前台探测命中后的补偿重试、重试去重、可见性门控与冲突暂停联动集中到同一条链路，减少分散判断。
...
- **长时间开启页面晚到补触发启动检查修复**: 启动同步回调若因页面挂起、后台恢复或延迟调度而错过“新鲜窗口”，现在只会顺延真正需要顺延的“每日首次同步”；不会再在旧页面上补跑“每次加载同步检查”或首次可见前台探测，减少“已经开了很久却突然提示云端有更新并刷新”的误触发。
- **自动同步图标遗留“后台同步中”修复**: 自动同步指示器的 `running` 展示改为严格依赖真实活动同步锁，不再因跨标签页遗留状态在锁已失效后继续长时间显示“后台同步中”。
- **自动检查云端更新偶发漏拉修复**: 当前台探测命中远端更新，但后续安全同步因锁占用、已有同步在执行或短暂失效被跳过时，会补偿重试而不是直接进入冷却，改善设备 B 偶发不自动拉取的问题。
- **阅读进度自动合并后页面无感修复**: 阅读进度冲突被自动合并并回写云端后，页面会按统一刷新策略处理，避免“已合并但页面没有明显变化”的体感问题。
- **手动智能合并仅读本地修复**: 线程页手动智能合并现在会先将合并结果回写远端，再导入本地并刷新，避免“当前页看似合并成功，但远端仍保持旧版本”的半完成状态。
