...
### ✨ 新增与增强
- **远程同步**:
    - **新增手动同步选择**: 当手动同步时检测到本地与云端数据不一致，会弹出选择框，清晰展示两端数据的最后更新时间，让用户自主选择“从云端拉取”或“推送到云端”，防止数据被意外覆盖。
    - **新增同步状态显示**: 在设置界面会显示上次成功同步的时间，帮助用户了解同步状态。
    - **新增远程同步配置教程页面**: 完善远程同步配置教程页面，并部署到 Netlify。
    - **阅读进度**: 新增阅读记录自动清理功能。允许用户在设置中开启该功能，并可以配置清理周期（如1个月、3个月、6个月或永不），以自动删除长时间未访问的帖子的阅读记录，保持数据清爽。
    - **远程同步**: 优化了远程同步设置中的教程与安全提示区域样式，将其从纯文本升级为带有图标的、更具设计感的提示框，提升了视觉一致性和信息清晰度。
    - **同步设置**: 在远程同步面板中增加“打开 Gist 页面”按钮，方便用户快速访问和管理自己的备份 Gist。

### 🎨 UI/UX 优化
- **远程同步**:
    - 优化了手动同步弹窗的 UI，使其更清晰易用。
    - 修正了同步信息中时间戳的显示样式。

### 🐛 修复
- **远程同步**:
    - **核心逻辑重构**: 重构了远程同步的数据变更检测机制，只有在数据实际发生变化时才会更新时间戳并触发上传，避免了仅保存设置而不修改内容时产生的无效同步。
    - **数据导入**: 修复了导入数据后，本地时间戳未同步更新的问题，确保了数据版本的一致性。
    - **设置清除**: 修复了使用“清除脚本数据”功能后，远程同步相关的 UI 状态不会立即更新的问题。

### 📦 重构
- **代码结构**: 将自动同步与手动同步的逻辑分离，使代码结构更清晰，维护性更高。

### 🎨 UI/UX 优化
- **远程同步**: 增强了远程同步设置界面的交互体验。当远程同步功能被禁用时，相关的输入框（Gist ID, PAT）和操作按钮（手动同步）会自动变为禁用状态并降低不透明度，使用户界面状态更清晰。

### ✨ 新增与增强
- **新增远程同步功能 (Gist)**:
    - **核心功能**: 新增了通过 GitHub Gist 在不同设备间双向同步所有设置和数据（屏蔽列表、用户标记、阅读进度等）的功能。
    - **自动同步**: 脚本启动时会自动执行一次同步。此外，当本地数据发生任何变更（如屏蔽用户、添加标记）后，脚本会在用户停止操作5秒后自动将改动推送到云端，实现了近乎无感的自动备份。
    - **数据安全**: 同步基于时间戳 (`lastUpdated`) 进行判断，确保始终保留最新版本的数据，避免旧数据覆盖新数据。
    - **手动控制**: 在设置面板中提供了“手动同步”按钮，方便用户随时强制执行同步。
    - **UI集成**: 在“设置同步”标签页中集成了完整的远程同步配置界面，包括 Gist ID 和 Personal Access Token 的输入。

### 🐛 修复
- **设置保存**: 修复了远程同步设置（Gist ID, PAT）无法正确保存的问题。

### 📦 重构
- **数据导出**: 重构了 `exportLocalData` 函数，使其能返回包含 `lastUpdated` 时间戳的数据对象，为同步逻辑提供支持。
- **数据保存**: 重构了所有核心数据的保存函数 (`saveSettings`, `saveBlockedUsers` 等)，使其在保存数据后能自动触发经防抖处理的远程同步推送。
- **DOM 观察器**: 增强了 `MutationObserver` 的稳定性，在目标容器 `#ct` 不存在时也能正常工作，避免了潜在的脚本错误。

### 📦 重构
- **重构“设置同步”功能**: 为即将推出的远程同步功能进行准备，将现有功能明确为“本地备份与恢复”，并更新了相关界面布局和文本说明。

### 🎨 UI/UX 优化
- **优化确认按钮样式**: 优化了屏蔽等操作的二次确认按钮样式，鼠标悬停时颜色更突出并增加图标提示，提升操作辨识度。
- 调整了设置面板的标签页顺序，将“通用设置”提前至首位，使其成为默认打开的标签页。
- 微调了“导航栏定制”标签页中的部分样式。

### ✨ 新增与增强
- **新增远程同步功能 (UI与配置)**:
    - 在“设置同步”标签页中，新增了“远程同步 (通过GitHub Gist)”模块的完整用户界面。
    - 实现了远程同步功能的配置管理，用户现在可以输入并保存他们的 Gist ID 和 GitHub Personal Access Token。
    - 新增了 `s1p-setup-guide.html` 教程页面，并将其纳入版本控制，为未来引导用户设置同步功能做准备。
- **在新标签页打开帖子**: 新增“在新标签页打开帖子”功能，允许用户在帖子列表页中直接在新标签页中打开帖子，并提供了“后台打开”的选项。
- **阅读进度**: 新增“后台打开”选项，当“在新窗口打开”启用时，允许用户在新的非激活标签页中打开阅读进度链接，避免打断当前浏览。

### 🐛 修复
- **链接打开行为一致性**: 修复了帖子列表页中的页码链接（如 `...2 3 4...`）不遵循“在新窗口打开帖子”设置的问题，现在其行为已与主标题链接统一。
- **置顶帖功能兼容性**: 修复了阅读进度跟踪、新窗口打开、关键字屏蔽、用户联动屏蔽等多项功能对置顶帖 (`stickthread`) 不生效的严重问题。
- **设置即时生效**: 修复了当更改“在新窗口打开阅读进度”或“后台打开”设置后，需要手动刷新页面才能让其生效的问题。现在相关设置会即时应用。

---

## [v4.5.0] - 2025-08-19
### Changed
- **重构帖子屏蔽交互**: 彻底重构了帖子列表页的屏蔽操作方式。
  - **引入操作菜单**: 将原有的悬浮“屏蔽”按钮替换为更紧凑、更现代的三点式操作菜单，避免了原先的布局平移效果。
  - **优化确认流程**: 在操作菜单中直接提供“确认/取消”选项，简化了屏蔽流程，提升了用户体验。
- **移除设置项**: 移除了已废弃的“帖子屏蔽按钮悬停延迟”设置选项。

### Fixed
- **修复屏蔽菜单关闭问题**: 为帖子屏蔽操作菜单的“取消”按钮增加了事件监听，彻底解决了在某些浏览器或特定交互下菜单无法正常关闭的问题。
- **修复布局问题**: 修正了因新增操作列而导致的帖子列表表头和分隔行错位的问题。
- **调整UI间距**:
  - 减小了帖子图标与表格左侧边框的间距。
  - 修正了用户标签编辑菜单的水平位置，使其对齐更精确。
### 修复
- **S1 NUX 兼容性**: 重构了对 S1 NUX 样式的检测逻辑，解决了脚本部分功能（如鼠标悬停显示评分）和 S1 NUX 样式冲突的问题。新的检测机制更可靠、性能也更高。
- 修复了在帖子详情页中，鼠标悬停在脚本添加的操作按钮（如“屏蔽用户”、“标记用户”）上时，会导致论坛原生的“更多选项”小三角错误消失的问题。
### 📚 文档
- 全面更新 README.md，使其内容与脚本的最新功能保持完全一致。
### 🎨 优化与重构
- **阅读进度**: 重构了帖子内部的阅读进度跟踪与保存机制，彻底解决了在快速滚动、手动跳转或关闭页面时，阅读进度偶尔不更新或记录不准确的问题。同时，移除了错误的进度回滚保护，现在脚本将始终忠实记录用户最后查看的位置，即使该位置比之前的记录更靠前。
