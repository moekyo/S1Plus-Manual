...
- **Style**: 优化 Toast 通知的视觉样式，使其更具辨识度。

### 🐛 修复 (Bug Fixes)

- **链接处理**: 修复了帖子列表底部的翻页按钮（包括上一页/下一页和页码）会被“在新标签页打开”功能错误处理的问题，确保其总是在当前页跳转。

### 🐛 修复 (Bug Fixes)

- **阅读进度跳转**: 修复了“阅读进度跳转”按钮的“在新标签页打开”行为不独立的问题。现在，此功能仅受其自身的设置开关控制，不再错误地依赖于全局链接总开关。

### 📦 重构与其他 (Refactoring & Other)

- **链接点击处理**: 优化了全局链接点击事件的处理逻辑，通过统一整合和设定优先级，提升了代码的可维护性。同时移除了阅读进度按钮上多余的独立事件监听器。

### ✨ 功能增强 (Features & Enhancements)

- **“在新标签页打开”功能重构与增强**:
  - **统一设置**: 将所有“在新标签页打开”的选项整合到通用设置中的一个独立区域，并由一个总开关控制，管理更清晰。
  - **功能细分**: 现在可以独立控制主导航栏、设置/通知侧边栏以及帖子/版块链接是否在新标签页中打开。
  - **后台打开**: 每个选项都支持独立的“在后台打开”设置。
  - **自动迁移**: 脚本会自动将旧版相关设置无缝迁移到新的数据结构，无需用户手动操作。

### 🐛 修复 (Bug Fixes)

- **悬浮控件交互**: 修复了增强型悬浮控件在收起状态下仍会遮挡并拦截下方页面元素点击的问题，提升了页面的交互体验。

### 📦 重构与其他 (Refactoring & Other)

- **代码格式化**: 对脚本代码进行了全面的自动化格式化，统一了代码风格，提升了可读性和可维护性。

### ✨ 功能增强 (Features & Enhancements)

- **“在新标签页打开”功能重构与增强**:
  - **功能扩展**: 此功能现在覆盖论坛内几乎所有链接（包括版块、用户空间等），而不再局限于帖子列表中的标题和页码。
  - **性能与稳定性**: 重构了底层实现，改用更高效、更可靠的全局事件委托机制，显著提升了性能，并能完美兼容 AJAX 翻页等动态内容。
  - **智能排除**: 新增了智能排除规则，可自动忽略登出、脚本UI内等特殊链接，防止意外行为。
  - **UI优化**: 更新了设置面板中的功能描述，使其更准确地反映功能的实际作用范围。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **设置界面微调**: 对设置面板中的部分 UI 元素进行了微调，移除了“用户标记”列表中不必要的分组标题，并调整了“设置同步”中导入/导出文本框的间距，使整体布局更简洁、协调。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **用户标记菜单交互优化**: 优化了帖子楼层中用户标记操作菜单的交互体验。现在，只有当鼠标悬停在操作图标上时才会触发菜单，并且菜单在鼠标移入后会保持显示，避免了意外关闭，操作更稳定、更精准。

### Style

- **设置面板 UI/UX 优化**:
  - 为整个设置面板设定了统一的字体族并启用抗锯齿，提升文本渲染质量。
  - 调整了主标题、分组标题、列表项标题和正文等各级文本的字重、尺寸和行高，构建了更清晰的视觉层级，提升了整体可读性。
  - 统一了设置项标签、描述文本、元信息等 UI 元素的样式，使其风格更加一致。
  - 优化了各区块的间距（margin/padding），使布局更加协调。
- **CSS 修复**:
  - 修正了 CSS 变量定义中一个缺失的分号。
...
- **界面风格统一**:
  - 移除了设置弹窗、上下文菜单、行内确认菜单等多个 UI 组件的边框，使其设计风格更加统一、简约。
  - 统一了“隐藏图片占位符”和“显示/隐藏本楼图片”按钮的样式，使其与脚本主按钮（`.s1p-btn`）的视觉风格完全一致，增强了界面的一致性。

### ✨ 功能增强 (Features & Enhancements)

- **防止对自己的帖子进行操作**: 现在脚本会智能识别当前登录的用户，并自动隐藏在用户自己的帖子楼层出现的“屏蔽用户”和“标记用户”按钮，避免了误操作。

### 🐛 修复 (Bug Fixes)

- **修正多账号自动签到逻辑**: 修复了在切换不同账号后，自动签到功能因共享签到状态而无法为新账号正常执行的问题。现在，签到记录会基于用户 UID 单独存储。

### 📦 重构与其他 (Refactoring & Other)

- **代码清理**: 移除了已废弃的“导航同步菜单”相关的 CSS 代码，保持了代码库的整洁。
- **逻辑优化**: 将 S1 NUX 主题的推荐间隔从 3 天延长至 7 天，减少了对用户的打扰。
- **函数增强**: 新增了 `getCurrentLoggedInUid` 辅助函数，用于更稳定、更准确地获取当前登录用户的 UID。


### 新增

- **与论坛黑名单双向同步**:
  - 在脚本中屏蔽用户时，可自动将其同步添加到论坛的官方黑名单中。
  - 在脚本中取消屏蔽用户时，也会自动从论坛官方黑名单中移除。
  - 此功能可在 **S1 Plus 设置 -> 用户屏蔽 -> 同步至论坛黑名单** 中开启或关闭。

### 优化

- **屏蔽用户列表**:
  - 为已成功同步到论坛黑名单的用户增加了“已同步”状态提示，方便识别。
  - 优化了“已同步”状态标签的视觉样式，使其更清晰、更美观。
- **操作反馈**:
  - 屏蔽和取消屏蔽用户的确认提示框中，会明确告知是否会同步到论坛黑名单。
  - 操作完成后的成功或失败消息也进行了相应优化，提升了交互清晰度。
- **界面微调**:
  - 统一调整了用户标记和回复收藏列表的内容字体大小，改善阅读体验。

### 修复

- 修复了在取消屏蔽已同步的用户时，因状态标签存在而无法正确读取用户名的问题。
- 修正了与论坛黑名单同步时网络请求的 `Content-Type` 标头拼写错误，提高了同步成功率。

### 🐛 修复 (Bug Fixes)

- 恢复了帖子列表中原生图标（如“新回复”图标）的显示，此前版本中该图标被意外隐藏。

### 📦 重构与其他 (Refactoring & Other)

- 清理了代码中残留的开发和调试注释，优化了代码的可读性。
- 清理了代码中残留的开发和调试注释，优化了代码的可读性。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- **界面**: 固化用户标记/导航编辑的“删除”按钮为 S1 Plus 经典样式，移除对 S1 NUX 样式的动态适配，并移除相关设置选项，简化代码逻辑。
- **界面**: 优化阅读进度提示条的出现/消失动画，使其更加平滑。
- **界面**: 新增深色模式下的样式微调：
  - 阅读进度条颜色调整为更柔和的橄榄绿。
  - 删除按钮图标在深色模式下默认为白色，悬停时隐藏。
- **设置**: 调整“阅读进度”设置项的布局，使“后台打开”作为“在新窗口打开”的子选项，逻辑更清晰。

### 🐛 修复 (Bug Fixes)

- **修复**: 修正了在设置界面中，部分功能开关无法正常展开/折叠其子选项的问题。

### 📦 重构与其他 (Refactoring & Other)

- **代码健康度提升**: 对脚本进行了全面的代码清理，移除了大量过时或冗余的开发注释、调试代码和空行，显著提升了代码的可读性和可维护性。
- **逻辑解耦**: 将“导航栏定制”功能开关的事件监听逻辑从其渲染函数中分离，并统一整合到全局的功能开关事件处理器中，降低了模块间的耦合度。
- **UI 结构优化**: 微调了“导航栏定制”编辑器的 UI 结构，增加了分组标题并统一了列表样式类名，使其与其他设置项的视觉和代码结构保持一致。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- 重构了设置面板中功能项的展开/折叠动画，改用更平滑、性能更好的 `height` 过渡动画，提升了交互的流畅度和现代感。

### 🐛 修复 (Bug Fixes)

- 修复了“导航栏定制”标签页的 HTML 结构与其它标签页不一致的问题，确保了功能开关的展开/折叠动画能够统一、正常地工作。
- 修复了设置面板中分段控件（Segmented Control）在折叠状态下（不可见时）无法正确计算宽度，导致初次展开时滑块位置不正确的问题。
- 统一了“阅读进度”功能的子选项显隐方式，将其重构为与其他设置项一致的 `s1p-feature-content` 折叠结构，确保了 UI 行为的统一性。
- 修复了在设置面板中关闭一个功能模块后，其下属的子设置项（如开关、输入框）在视觉上没有同步变为禁用状态的问题，提升了 UI 的一致性和用户体验。
- 修复了帖子楼层号（如“1 楼”、“2 楼”）在新版布局下偶尔出现初始闪烁或短暂消失的问题。通过引入 `.s1p-layout-ready` 类和 `visibility` 控制，确保所有布局操作完成后再显示楼层号，彻底消除视觉跳变。

### ✨ 新增与增强 (Features & Enhancements)

- 启用全新的“每日首次同步”与“常规后台同步”分离逻辑，主流程会优先执行每日首次同步（如有必要自动刷新页面），否则再执行常规后台同步检查，保证同步体验更智能、稳定。
- 新增 `handlePerLoadSyncCheck` 函数，支持“每日首次同步”关闭时依然可用的后台自动同步。
- `handleStartupSync` 逻辑重构，优化同步锁与冲突处理，提升多标签页下的同步安全性。

### 📦 重构与其他 (Refactoring & Other)

- 增强了功能开关的动画处理逻辑，确保在复杂的 DOM 结构下也能安全、可靠地执行高度过渡动画。
- 重构了设置面板中主要功能模块（如用户屏蔽、标记等）的总开关逻辑。现在，关闭一个功能会彻底地从页面 DOM 中移除其对应的 UI 元素（如操作按钮），而不仅仅是隐藏，使功能的禁用和启用更加干净、彻底。
- 优化 `addActionsToSinglePost` 布局函数，确保无论容器是否已存在都能正确显示楼层号。
- 主流程 `main` 结构调整，分步执行同步与初始化，提升健壮性。

### ✨ 新增与增强 (Features & Enhancements)

- 关键字屏蔽规则的删除操作现在会立即生效，无需再手动点击保存，提升了操作的即时性和流畅性。
- 为 S1 NUX 推荐弹窗的按钮增加单独高亮颜色，提升视觉反馈。

### 🎨 UI/UX 优化 (UI/UX Improvements)

- 对增强型悬浮控件（返回顶部/底部等）进行了全面的视觉重构，采用了更小巧、更精致的无边框设计，并优化了图标大小和悬停交互效果，使其更具现代感。
- 对行内确认菜单（如删除用户标记）的样式进行了全面微调，包括减小尺寸、间距和字体大小，使其外观更精致、布局更紧凑。
- 优化了用户标记操作菜单的高度和内边距，确保了菜单项与帖子楼层内的其他 UI 元素在视觉上更完美地对齐。

### 🐛 修复 (Bug Fixes)

- 优化了远程同步失败时的错误提示，现在能够从 Gist API 的响应中提取并显示更具体的失败原因，便于用户排查问题。
- 修正了删除关键字屏蔽规则时的确认提示信息，使其准确反映出“立即生效”的行为，避免用户混淆。
- 修正了行内确认菜单的智能定位逻辑，特别是优化了其在屏幕边缘的左侧定位算法和垂直对齐方式，确保菜单在任何情况下都能稳定、精确地显示。

### 📦 重构与其他 (Refactoring & Other)

- 重构了悬浮控件相关的 HTML ID 和 CSS 类名，统一了命名规范，提升了代码的可读性和可维护性。
- 改进通用确认弹窗函数，使其支持自定义样式类，增强了组件的灵活性。

### 📝 文档优化 (Documentation)

- 丰富了“云端同步”功能的描述，明确强调同步过程采用 SHA-256 哈希校验，提升数据安全性与完整性说明。
- 增加了“手动同步高级模式”与同步状态反馈的详细说明，帮助用户理解同步按钮的多种交互方式。
- 对“界面与导航定制”部分补充了 S1 NUX 主题兼容模式、增强型悬浮控件等新特性说明。
- 优化了“功能特点”与“使用说明”结构，使各项功能描述更清晰、条理更分明。
- 修正了部分术语、排版和链接细节，使文档内容与实际功能保持一致。
