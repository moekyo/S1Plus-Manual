...
- **单一主题权威**：S1 Plus 只维护一份运行时主题状态（浅色/深色），再投影为 `html.s1p-theme-light` / `html.s1p-theme-dark`；样式与脚本统一消费这份状态，不再各自读取系统主题，避免 NUX 与系统主题不一致时自有界面配色错乱。
- **NUX 开关双向收敛**：NUX 启用状态改为每次从渲染后的 `NUXISENABLED` 标记重新读取，开启与关闭都能收敛，不再出现关掉 NUX 后仍停留在 NUX 配色的情况。
- **NUX 自定义主题识别**：存在有效 `--darktheme` 时以它为准；缺失时按实际渲染的调色板（`--bg` / `--t` 等）保守推断深/浅色，无法判定时按浅色处理，不再默默跟随操作系统。
- **`prefers-color-scheme` 角色收窄**：仅作为 NUX 自身自动主题 CSS 的同步触发信号，不再作为 S1 Plus 的主题判定来源。
- **深色样式改造**：原先按 `@media (prefers-color-scheme: dark)` 编写的深色规则改为挂到 `html.s1p-theme-dark`，覆盖设置面板、调试面板、搜索、浮动控件、按钮与菜单等 S1 Plus 自有界面。
...
- **单一调度入口**：初始化、resize、ResizeObserver、字体加载完成统一进入同一个调度器；同步按钮与同步状态条的就地变更也改为通过通知入口请求重算，不再由多个回调各自改写导航 DOM 状态。
- **测量契约与分区模型边界**：测量结果新增「容器宽度 / 固定区域宽度 / 可用宽度 / 真实渲染宽度」诊断契约；header 行不是 flex/grid 容器时退回真实渲染宽度；真实宽度小于分区预算时以真实几何为准，避免主题引入未建模的 `gap` / `margin` 导致裁剪。
- **测量态特异性**：`.s1p-nav-measuring` 选择器提高特异性，避免主题的 `!important` ID 规则让「需求宽度」退化成上一轮 flex 分配结果。
...
- **按服务端结果确认成功**：返回内容明确包含 `签到成功` / `已签到` 等状态后才写入成功记录，避免把 2xx 错误页误判为签到成功。
- **失败退避定时重试**：失败后除保留 `nextRetryAt` 跨页面恢复外，当前页面也会按指数退避自动重试。
- **跨标签页串行**：同一 UID 使用 Web Locks 串行化多标签页签到请求，不支持 Web Locks 时回退到现有尝试锁。
- **按论坛时区日切**：签到日期改用论坛 GMT+8 计算，减少浏览器本地时区差异导致的跨日误判。
- **降低页面结构依赖**：优先使用页面签到链接；链接缺失时尝试从 `formhash` 构造签到地址。
- **UID 缓存刷新**：自动签到读取当前页面 UID 时强制刷新，避免同页切换账号后沿用旧 UID。

### 🩹 同步修复

- **前台待处理记录不再无限重跑同步**：修复旧版遗留的待处理记录被一次普通写入升为 `storageVersion: 3` 却未补上不可变 `registrationClock` 的问题。该形状会让此后所有 mutable 写入与终态写入都被 fail-closed 守卫拒绝，记录永久无法结算，并在每次回到前台/重新打开页面时重跑一整轮远端同步、反复点亮导航栏待处理。现在升级记录时会同时分配身份；已经无法结算的遗留记录会被安全放弃而不是永久保留（仍拒绝为其伪造终态）。

---

## [v7.0.0] - 2026-09-20

- 跨标签页锁诊断回执改用独立记录，避免并发覆盖；统一导出最近 7 天内最多 256 条回执，并标明旧环境回退机制。

### 🧭 手动同步恢复与日志增强

- **手动同步排队恢复**：同步锁自然过期或释放通知缺失时，排队的手动推送/拉取会自动重新检查并弹出确认框；续租期间继续等待，不抢占正在执行的同步。
- **完整诊断事件**：关键事件在调试面板关闭时也会保留；同步、网络、设置、存储、导入、阅读进度、初始化及页面功能投影记录结构化详情和操作关联。日志支持模块/操作筛选、操作摘要、完整诊断导出及多份导出合并。
- **占锁证据与降噪**：区分手动请求优先阻塞、执行锁占用和竞争校验失败；记录持有者、执行 ID、阶段、续租、释放、页面离开和观察到的 TTL 过期。同类等待合并为有次数和首末详情的记录，不再把“未执行”说成“同步完成”。
- **有界保留**：日志按条数和容量限制保留，优先保留近期失败操作及其已有前文；自动隐藏凭据和正文等字段，明确标注截断、淘汰和保存失败。详细诊断可临时开启 10 分钟，旧同步摘要批量保存。
...
- **默认放大显示宽度可调**: 通用设置中“放大显示”默认模式新增“铺满宽度比例” slider，可在 60% - 100% 间调整默认打开宽度；选择“完整显示”时自动隐藏该控件，并在说明中区分设置比例与查看器顶部显示的图片实际缩放百分比。

### 🐛 阅读进度修复

- **阅读记录跳转定位修复**: 阅读进度跳转进入帖子详情页后，会在 `#pid...` / `#post_...` 楼层锚点基础上主动校正滚动位置，并动态避开页面顶部固定/吸顶菜单栏，避免目标楼层停在页面顶部或被顶部工具栏遮挡。

### 🪟 弹窗玻璃与层级系统重构 (Dialog Glass and Layering Refactor)

- **弹窗内容壳统一**：新增 `.s1p-dialog-content` 通用内容壳与 compact、default、wide、expanded 四档尺寸，取代确认弹窗、Token 配置和阅读记录详情原有的专属外壳类。
- **设置二级弹窗标准化**：Token 有效期、阅读记录详情、确认与输入弹窗、用户屏蔽和手动同步弹窗统一挂载到设置面板内容容器，复用相同的二级层级、玻璃材质与进出场动画。
- **一级通用弹窗统一**：独立打开的确认、输入和高级确认弹窗复用同一内容壳与尺寸系统，并根据一级语义自动采用确认弹窗玻璃预设。
- **玻璃材质集中管理**：一级设置面板、一级确认弹窗与设置二级弹窗分别通过共享材质预设管理背景、阴影和滤镜，避免业务弹窗重复定义外壳视觉。
- **旧样式与动画清理**：移除 Token 配置的独立玻璃样式和旧专属内容壳，并将原先带有全屏含义的动画辅助函数改为通用分层弹窗命名。
...
- **弹窗预览补全**: UI 组件展示区的「弹窗预览」补齐版本欢迎、S1 NUX 推荐、手动同步选择、启动同步冲突、Token 过期提醒、Token 日期配置、阅读记录详情等真实弹窗场景；打开预览时临时插入可读性测试背景，并在切换预览前清理旧弹层，避免重复点击叠加影响视觉判断。
...
- **导航栏直接拉取/推送优先级修复**: 导航栏同步按钮菜单里的“拉取”“推送”现在作为显式手动覆盖操作处理；点击后会取消当前自动同步、待同步队列、冲突暂停、相关锁与重试退避，再按用户指定方向执行，避免冲突态或同步中状态挡住用户的最终选择。
...
- **隐藏/冻结 owner 自动推送延迟修复**: shared background scheduler 的 owner 标签页进入后台时不再单纯依赖可能被冻结的 `setTimeout`；页面隐藏、storage 接管、owner lease 过期恢复，以及隐藏页自己登记/恢复 owner 时都会补发 pending 推送。可见的非 owner 标签页现在也会观察 owner 租约，到期未续租就接管并立即执行已过期的阅读进度推送，避免导航栏长期停在待推送箭头但调试日志没有推进，或切回前台后才看到推送动画。
- **pending 覆盖后的 owner 恢复修复**: `recoverPendingAutoSyncIfNeeded` 判断 pending 已被 shared debounce 覆盖时，也会触发 shared owner lease 观察；可见页不再因为“已覆盖”就放弃接管隐藏/冻结 owner，避免阅读进度推送等到切回列表页才进入运行态。
- **隐藏页阅读进度推送收口修复**: 页面隐藏/卸载现在统一进入 sync lifecycle checkpoint，先 finalize 阅读进度等本地变更，再重新读取 shared scheduler 状态；隐藏页会立即补发可执行的 pending 推送，若被锁占用或即将卸载则释放 owner 交给其他页面接管，避免依赖可能被冻结的下一轮 `MessageChannel` / `setTimeout`。
...
- **导航栏待推送误显示运行中修复**: 本地存在待推送变更时，导航栏不再把其他标签页的新鲜后台同步锁显示成推送中动画；只有当前标签页作为 Live Runner 才进入运行态，避免切换标签页后从动画回落到静止待推送。
- **手动阅读进度合并半完成状态修复**: 线程页手动阅读进度合并会先把合并结果回写远端，再导入本地并刷新，避免“当前页看起来合并成功，但远端仍是旧数据”。
...
- **启动同步阅读进度误报冲突修复**: 启动同步现在会先分类本地变更；若确认除阅读进度外的基础数据一致，会直接走阅读进度自动合并并回写云端，不再把“刚看帖产生的阅读进度待同步”升级为全局同步暂停。设置、屏蔽、收藏等核心数据较新时仍保留启动安全暂停。
...
- **标签页标题同步状态降噪**: 纯 metadata probe 和明确的远端拉取不再让后台标签标题短暂显示同步状态；无变化、冷却跳过或等值复查时标题保持原样，只有本地推送、推送失败或需要处理的冲突才显示标题同步状态。标题侧还会识别 source mismatch 导致的“拉取被显示态默认成后台推送”情况，避免误点亮标题；导航栏同步指示器和实际同步流程不受影响。
...
- **共享后台调度延后日志节流**: 共享调度因同步锁占用而 +1s 重试时，同一调度代次和原因 1 秒内最多记录一条"共享调度已延后"日志，避免锁冲突期间刷屏；调度重试逻辑不变。
...
- **导航栏云端确认图标语义校准**: 推送完成后的云端确认、等值更新时间复查，以及前台探测命中变化但暂缓 follow-up sync 的复查等待，都统一显示放大镜确认态，不再复用待推送箭头或中性三点，避免被误读成“又有本地内容待推送”。
- **标签页标题同步状态交接修复**: 标题上的成功/失败/冲突结果现在可在原 Title Owner 关闭后交接给其它后台 S1 标签页；`[同步中]` 只允许真实 Live Runner 显示，旧 runner 关闭留下的新鲜锁不再导致其它标签页出现幽灵同步中，也不会回退显示旧结果。
- **导航栏推送/拉取箭头过渡优化**: 推送/拉取 pending 图标从 `scale(1.3)` 收敛到 `scale(1.25)`，并与 running 共用同一组三箭头 SVG。pending 切换到 running 时会把双箭头缩放到 running 队列相位并接入第三枚箭头，减少从静态待处理到流动同步中的替换感和反向回退感。
- **导航栏同步方向图标视觉校准**: pending 推送/拉取图标不再通过半透明弱化颜色，保持与手动同步按钮一致的主题色；放大镜 probe 切换到推送/拉取 running 时改用无旋转、无横向位移的专用转场，避免慢放下出现左右摇摆。
- **指示器调试面板校准**: 指示器调试 tab 新增“实际显示/预览显示”字段，并把常看转场升级为逐步状态序列，补齐 pending push/pull → running 与 probe → pull 的固定预览入口，便于复核最新导航栏同步图标流转。
...
- **弹窗与浮层磨砂玻璃风格统一**: 设置面板、确认弹窗、图片查看器、日期选择器、用户标记编辑器、文档型帮助浮层、Toast 和右侧浮动控制统一为分层玻璃语言；全屏蒙版取消黑色遮罩变量，改为无色 blur，避免图片查看器等顶层弹窗独立打开时发暗。
- **全屏蒙版模板边界收口**：顶层设置面板、独立确认类弹窗和图片查看器使用 `s1p-fullscreen-modal` 蒙版模板；Token 有效期、阅读记录详情及设置内确认弹窗改由 `.s1p-modal-content` 承载，不再额外叠加全局蒙版。
- **分层弹窗动效时序优化**：一级与设置二级弹窗统一采用“打开先蒙版后内容、关闭先内容后蒙版”的队列式时序，并让内容壳的进退场缩放终点一致。
- **弹窗内容壳材质收口**：一级与设置二级弹窗统一使用 `.s1p-dialog-content` 结构类，分别应用一级确认玻璃和设置二级玻璃预设，尺寸、滚动与材质职责不再混在业务专属类中。
- **设置承载二级弹窗磨砂样式统一**：Token 有效期、阅读记录详情、设置内确认与输入弹窗、用户屏蔽和设置内手动同步选择统一使用标准二级弹窗容器；移除阅读记录表格斑马纹，并让深浅色文字跟随脚本主题色。
- **图片查看器切换按钮默认阴影减弱**: 图片查看器上一张/下一张按钮新增专用默认态阴影变量，浅色和深色分别校准阴影强度；hover、背景和模糊效果保持不变。
- **滑块数值胶囊深色模式校准**: 通用 range slider 的百分比数值胶囊新增 `--s1p-range-value-bg` 背景变量；深色模式改用浅灰半透明背景，避免图片查看器设置中的数值胶囊过深。
- **右侧浮动控制玻璃质感校准**: `s1p-floating-controls-wrapper` 的收起把手与展开按钮改用调试面板收起 FAB 同款低透明玻璃背景、blur 与阴影，避免悬浮控件在浅色主题下显得偏白偏亮。
- **弹窗玻璃可读性回归修复**: 全局下调全屏蒙版 blur，并把 Dialog / Popover / 浮动控件改为更实背景与轻量滤镜变量；随后略微加实承载文字的玻璃背景，同时把 Toast 调得更通透、略增 toast 专用磨砂，图片查看器恢复修改前样式。手动同步选择/冲突、欢迎弹窗、Token 提醒、阅读记录详情等共享确认弹窗统一使用 dialog 文本 token，避免背景论坛文字透进弹窗正文和同步对比表格。
- **图片查看器玻璃化校准**: 图片查看器面板壳改为透明结构层，只负责圆角裁切、阴影和动画；工具栏与图片舞台各自铺设玻璃背景，避免父子背景 alpha 叠加导致后续调参互相影响。
- **图片查看器透明感收敛**: 校准工具栏和图片舞台的独立背景 alpha；图片舞台改为稳定底色层 + 独立轻磨砂层，浅色和深色保持同一处理，避免高对比白底内容被糊进舞台，同时保留透明图、暗色图的判断稳定性。
...
- **轻量浮层材质统一**: 提取 `--s1p-floating-surface-*` 与 `.s1p-floating-surface` 作为 tooltip、用户标记编辑器、日期选择器、确认型浮层、标签菜单和 Toast 的共享轻磨砂表面；带备注确认菜单外层改为透明布局层，避免两个子浮层之间被父级磨砂背景连成整条。导航栏拉取/推送行内菜单保留独立的更轻 surface 变量以贴近 tooltip 质感。
- **用户标记颜色深浅模式修复**: 用户标记和历史标记的文字色改为通过 `--s1p-tag-*-text` 变量控制，浅色模式彩色标签统一白字，深色模式统一 `#422006`，避免深色主题下亮色标签文字对比不足。
- **用户标记保存后工具栏原生链接丢失修复**: 保存或删除用户标记刷新楼层工具栏时，会先把“只看该用户 / 显示全部楼层 / 正序倒序浏览”等已移入脚本工具栏的原生链接归还到 `.authi`，再重建脚本按钮，避免相关图标保存后消失、刷新页面才恢复。
...
- **同步设置区信息架构重组**: “设置同步”页按数据方向拆为“手动同步”“自动推送”“自动拉取”“状态显示”“GitHub 连接”等分组；“检查策略”与“本地变更后自动后台同步”不再平铺混在一起，“持续可见时低频复查”也只在“回到前台”策略下显示。
- **导航栏同步按钮菜单常驻**: 导航栏同步按钮点击或悬停都会打开同一个“推送/拉取”菜单，点击作为键盘/触摸等场景的备用入口，不会直接执行同步；不再通过设置开关切换交互模式。旧的 `syncDirectChoiceMode` 设置会在归一化迁移时清理。
- **自动检查模式说明文案优化**: 将"回到前台检查"与"持续可见时复查"的模式描述文案拆分，移除各模式说明中已由独立开关控制的"页面持续可见时"表述，减少用户困惑。
- **自动推送设备 ID 交互收紧**: 同步设备 ID 移至“自动推送”开关下方；开启自动推送时若未填写会立即提示，提示消失后仍为空则自动回退开关，保存时继续兜底校验。自动拉取和手动同步不强制填写设备 ID。
