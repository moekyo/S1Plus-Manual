...
### 🐛 修复
- **自动签到**: 修复了自动签到成功后“打卡签到”链接不会立即消失的问题。现在链接会在签到后或当天再次访问时自动隐藏。
- **脚本健壮性**: 为自动签到功能增加了错误捕获，防止其在特定情况下因异常而导致整个脚本失效。

### ✨ 新增与增强
- 在帖子楼层底部的工具栏中（“只看该作者”链接后），为屏蔽用户功能新增一个快捷入口。

### 🎨 UI/UX 优化
- **帖子屏蔽交互**：
    - **动画效果**: 优化了屏蔽按钮的出现动画，现在仅移动图标和标题，避免了整行内容的晃动。
    - **悬停延迟**: 为屏蔽按钮增加了 1s 的悬停延迟，解决了因鼠标快速划过导致的误触和点击目标“逃跑”的问题，大幅提升了交互体验。
    - **垂直对齐**: 修复了屏蔽按钮在行内垂直方向上没有完美居中的问题。
- **二次确认按钮 (✓/✗) 重新设计**:
    - **图标化**: 使用现代化的 SVG 矢量图标替换了原有的纯文本字符。
    - **样式更新**: 将按钮形状从圆角矩形改为圆形，并移除了实色背景，改为图标本身带颜色（绿/红），鼠标悬停时会显示一个淡淡的背景色作为反馈，整体风格更轻盈、更美观。

### 🐛 修复
- **屏蔽引用**: 修复了被屏蔽引用的“点击展开”按钮显示为浏览器默认链接样式的问题，通过将其 HTML 标签从 `<a>` 改为 `<span>`，确保了其样式与脚本整体风格一致。

### ✨ 新增
- 在设置面板中新增独立的“通用设置”标签页。
- 在“通用设置”中增加“启用阅读进度跟踪”的开关。
- 新增“默认隐藏帖子图片”功能，可在“通用设置”中开启。当开启后，帖子内的图片默认将被占位符替换，需要手动点击才能显示。

### ⚡️ 变更
- 将原“界面定制”标签页重命名为“导航栏定制”。
- “通用设置”中的选项（如修改Logo、隐藏黑名单提示等）现在会实时自动保存。
- 优化了设置面板中功能开关的展开/折叠动画，使其更平滑、更自然。
- 更新了默认的导航栏链接，加入了外野、手游、VTB等常用板块。
- 优化了导航栏设置的加载逻辑，现在更新脚本不会覆盖用户已有的自定义导航栏。
- “导航栏定制”中的“恢复默认”按钮现在仅重置导航栏设置，不再影响其他全局设置，并重命名为“恢复默认导航”。

### 🐛 修复
- 恢复了“导航栏定制”功能区中的“保存设置”按钮，解决了部分设置无法保存的问题。
- 修复了在“导航栏定制”中，“添加新链接”时，新项目的样式与现有项目不一致的问题。

### ✨ 新增与增强
- **新增精细化数据清除功能**: 在 "设置同步" -> "危险操作" 中，新增了按功能模块单独清除本地数据的选项（如“屏蔽的用户”、“用户标记”、“阅读进度”等），取代了过去只能“一键清除所有数据”的按钮，提供了更灵活、安全的数据管理方式。

### 🐛 问题修复
- **阅读进度**: 彻底修复了新增回复数计算不准确的问题。现在通过记录上次阅读的**楼层号**，能够精确计算并显示自上次阅读以来的真实新增回复数。
- **阅读进度**: 修复了因旧版阅读记录缺少楼层号而导致跳转按钮显示为 `#undefined` 的问题。

### 🎨 UI/UX 优化
- **阅读进度**: 为帖子列表中的“跳转”按钮增加了图标，使其功能更直观。
- **阅读进度**: 优化了“跳转”按钮的图标样式，并确保图标颜色能与文字颜色保持同步。
- **阅读进度**: “跳转”按钮现在会显示更详细的信息，格式为 `P<页码>-#<楼层号>`，方便快速了解上次阅读的位置。
- 优化了设置面板中功能开关（如帖子屏蔽、用户屏蔽、用户标记等）的展开/折叠动画，使其更平滑、更自然。
