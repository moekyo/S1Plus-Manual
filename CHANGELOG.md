...
- **隐藏标签页自动推送延迟修复**: shared background scheduler 的 owner 标签页进入后台时不再单纯依赖可能被冻结的 `setTimeout`；页面隐藏、storage 接管或 owner lease 过期恢复时会补发 pending 推送，避免阅读进度等待到切回原列表页才开始同步。
