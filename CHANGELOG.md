...
- **隐藏/冻结 owner 自动推送延迟修复**: shared background scheduler 的 owner 标签页进入后台时不再单纯依赖可能被冻结的 `setTimeout`；页面隐藏、storage 接管或 owner lease 过期恢复时会补发 pending 推送。可见的非 owner 标签页现在也会观察 owner 租约，到期未续租就接管并立即执行已过期的阅读进度推送，避免导航栏长期停在待推送箭头但调试日志没有推进。
