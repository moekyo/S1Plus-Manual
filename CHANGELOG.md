...
-   **新增远程同步功能 (通过 GitHub Gist)**：
    -   **核心同步**: 实现了通过 GitHub Gist 在不同设备间双向同步所有脚本数据的功能，包括屏蔽列表、用户标记、阅读进度及各项设置。
    -   **自动同步**: 脚本启动时及本地数据变更后（经5秒防抖处理）会自动与云端同步，确保数据实时备份。
    -   **手动同步与冲突选择**: 当手动同步检测到数据不一致时，会弹出选择框，清晰展示两端更新时间，让用户自主选择拉取或推送，防止数据意外覆盖。
    -   **状态显示**: 在设置界面会显示上次成功同步的时间，方便用户掌握同步状态。
    -   **便捷访问**: 在远程同步面板中新增“打开 Gist 页面”按钮，方便快速访问和管理备份。
-   **阅读进度自动清理**: 新增阅读记录自动清理功能，用户可在设置中配置清理周期（1个月、3个月、6个月或永不），以自动删除陈旧的阅读记录。

### 🎨 UI/UX 优化 (UI/UX Improvements)

-   **远程同步界面**:
    -   增强了设置界面的交互体验，当功能被禁用时，相关输入框和按钮会自动变为禁用状态，使界面状态更清晰。
    -   优化了手动同步弹窗的 UI、时间戳显示样式以及教程提示区样式，提升了视觉效果和信息辨识度。
-   **弹窗样式**: 微调了用户屏蔽和用户标记功能的二次确认弹窗样式。

### 🐛 修复 (Bug Fixes)

-   **远程同步**:
    -   修复了远程同步配置（Gist ID, PAT）无法正确保存的问题。
    -   修复了仅保存设置但未修改内容时仍会触发无效同步的问题，现在只有数据实际变更时才会更新时间戳并上传。
    -   修复了导入数据后，本地时间戳未同步更新导致版本判断不一致的问题。
    -   修复了使用“清除脚本数据”功能后，远程同步相关的 UI 状态不会立即刷新的问题。

### 📦 重构 (Refactoring)

-   **同步逻辑**:
    -   重构了“设置同步”标签页的 UI 和代码，为远程同步功能打下基础。
    -   将自动同步与手动同步的逻辑分离，使代码结构更清晰。
    -   重构了数据导出 (`exportLocalData`) 与所有核心数据的保存函数，以支持自动触发远程同步。
-   **核心稳定性**: 增强了 DOM 观察器 (`MutationObserver`) 的稳定性，避免了在特定页面环境下可能出现的脚本错误。
...
### ✨ 新增与增强 
-   **新标签页打开帖子**: 新增“在新标签页打开帖子”功能，并提供了“后台打开”的子选项，允许用户在帖子列表页中更灵活地打开帖子链接。
-   **阅读进度后台打开**: 为“在新窗口打开阅读进度”功能增加了“后台打开”选项，可在不打断当前浏览的情况下打开进度链接。

### 🎨 UI/UX 优化 
-   **帖子屏蔽交互重构**: 彻底重构了帖子列表页的屏蔽操作，使用更紧凑、更现代的三点式操作菜单替代了原有的悬浮按钮，并优化了菜单内的确认流程，提升了操作体验和界面整洁度。
-   **设置面板调整**:
    -   将“通用设置”调整为默认打开的标签页。
    -   移除了因交互重构而废弃的“帖子屏蔽按钮悬停延迟”设置项。
    -   微调了“导航栏定制”标签页中的部分样式。
-   **视觉细节优化**:
    -   优化了二次确认按钮的样式，增加了鼠标悬停效果和图标提示。
    -   调整了帖子图标、用户标签编辑菜单等UI元素的间距和位置，使其更精确美观。
...
-   **交互与功能修复**:
    -   **阅读进度**: 重构了进度跟踪与保存机制，彻底解决了快速滚动或关闭页面时，进度偶尔记录不准确或丢失的问题。
    -   **屏蔽菜单**: 修复了屏蔽操作菜单的“取消”按钮在特定情况下无法正常关闭的问题。
    -   **设置即时生效**: 修复了“在新窗口打开”相关设置在修改后需要刷新页面才能生效的问题。
    -   **链接行为一致性**: 修复了帖子列表中的页码链接不遵循“在新窗口打开”设置的问题。
    -   **原生UI冲突**: 修复了鼠标悬停在脚本按钮上时，会导致论坛原生的“更多选项”三角图标错误消失的问题。
-   **布局修复**: 修正了因新增操作列而导致的帖子列表表头和分隔行错位的问题。

### 📚 文档 

-   全面更新了 `README.md`，使其内容与脚本的最新功能保持完全一致。
