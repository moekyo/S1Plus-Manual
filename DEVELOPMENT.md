...
- `s1pSyncLifecycleAdapter` 是同步系统唯一的浏览器生命周期入口。`visibilitychange`、`pageshow`、`pagehide` 和 `beforeunload` 只绑定一次，再由 `handle()` 映射为 hidden、visible、pageshow、pagehide、beforeunload phase。每个 phase 都先运行 sync lifecycle checkpoint，使本地 finalizer（尤其是阅读进度）先落盘，再执行 Scheduler flush/recover/handoff；visible/pageshow 随后才重读前台存储快照、恢复 Pending Dirty、执行远端 freshness probe 和刷新可见页 polling。pagehide/beforeunload 不接管 Running Sync，也不删除其锁。卸载 handoff 使用 generation fence 阻止当前页被自己的 GM change callback 立即选回 owner；如果 `beforeunload` 被取消，只有仍存活且可见的页面才会在下一轮 task 恢复 Scheduler Owner。
