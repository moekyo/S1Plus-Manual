...
- 导航栏直接拉取 / 推送的手动覆盖路径
  - `test-manual-sync-intent-priority.js` 覆盖立即执行、活动锁下 durable intent、`Pending(push|pull)` 投影、边界后确认、最新方向、单一确认 owner、设置关闭、精确取消与主动 dismiss 不误取消，并通过 deterministic interleaving 锁住 observed-head stale release/confirm、executing 单调性、terminal 不复活与 user-confirm sibling arbitration
