...
- 自动同步结果调用者必须通过 `syncResultPhasePolicy.handle()` 消费 Result Phase，不得各自重新判断 success / failure / conflict / skipped。`s1pCreateSyncResultPhasePolicy()` 只用于构造生产 module 和注入边界 adapter 的场景测试；刷新文案、冲突弹窗内容与实际 timer/reload 仍由各来源 adapter 负责。
