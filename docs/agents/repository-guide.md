...
- Keep `.s1p-modal-content`, `.s1p-glass-panel`, and `.s1p-confirm-content` as structural/semantic classes. The first-level glass class owns background, shadow, and filter; the structural classes should keep layout, sizing, scrolling, text, and motion behavior.
...
- Background, startup, and foreground follow-up sync share the `runRunningSync()` lifecycle. `runTransaction` must not resolve until baseline/writer state is persisted and covered pending/shared work is cleared; the lifecycle intentionally accepts no separate finalizer. Indicator aggregation, refresh, messages, conflict UI, and retry scheduling run only after both locks are released. Manual sync reuses the mode-lock implementation but keeps its explicit preemption flow separate.
...
- `pendingDirtyScheduler` is the only caller/test interface for Pending Dirty, shared generation, Scheduler Owner/lease, due timers, covered cleanup, and shared-vs-local retry selection. Construct it once with clock/tab/settings/timer adapters, then use semantic operations instead of passing owner, lease, generation, or timer mechanics per call. Keep owner/timer/coverage helpers inside its implementation; scenario tests use the same interface over the harness's in-memory GM adapter. Covered cleanup must revalidate before deletion and recover newer `last_modified` as Pending Dirty after a cross-tab delete race.
