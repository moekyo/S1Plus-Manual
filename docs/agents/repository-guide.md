...
- Registration identity is carried through normalized v3 pending values, attempt/settlement calls, mutable records, and terminal fences. Terminal writes use the captured `registrationClock` (or the same request's existing record as fallback) and never reconstruct it from the non-authoritative current marker. A v3 record missing that clock fails closed for mutable/terminal writes; legacy/v2 records remain readable and settle using their fixed registration timestamp/requestId ordering. Concurrent terminal candidates for one request are retained until watermark compaction keeps the winning evidence keys, so a late weaker mirror write cannot erase a stronger terminal fence. The current marker remains a compatibility/allocation hint, not terminal identity or authority.
