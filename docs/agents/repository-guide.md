...
- Navbar direct pull/push (`handleForcePull` / `handleForcePush`) is an explicit manual request, not an automatic recovery path. It still obeys the shared execution lock: when another run owns a valid lock it reports busy and leaves that run's locks, heartbeats, durable foreground intent, and retry ownership untouched. After exclusion is available it may invalidate this tab's stale unsent retries and hand off local scheduling before acquiring the manual lock. A successful pull/manual-sync may clear only the exact foreground intent generation captured at its start and only after the persisted baseline covers that generation's `remoteUpdatedAt`; a newer request stays pending even when its timestamp is equal. Keep `pagehide` / `beforeunload` recovery conservative: wait for lock expiry and rerun from fresh snapshots.
