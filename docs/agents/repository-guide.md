...
- Navbar direct pull/push (`handleForcePull` / `handleForcePush`) is an explicit manual request, not an automatic recovery path. `s1pCreateManualSyncIntentCoordinator()` is the display/request authority for these two directions: no active execution means immediate execution; an active valid mode/global lock creates durable `queued_waiting_for_execution_boundary` intent and the navbar shows the requested direction as `Pending`, never as a busy drop. After the lock boundary is free, one cross-context lease owner presents `awaiting_confirmation`; the dialog itself holds no execution lock, and confirmation re-reads the exact `requestId + generation` plus its `confirmationOwnerToken` before preparing `executing`. Every canonical transition also carries the caller-observed `expectedHeadEventId` and, where applicable, owner/execution fencing; a changed head is stale/no-op and is never silently rebased. The force handler is called only after the appended executing event is still the canonical resolver head. The canonical manual state is an append-only set of per-request/per-transition GM records enumerated with `GM_listValues`; the legacy shared key is only a change-notification mirror. The latest explicit direction replaces an older intent by deterministic authority ordering (`authorityRequestedAt`, `authoritySourceContextId`, `authorityRequestSequence`, `authorityRequestId`), while `generation` remains exact identity evidence rather than a global counter. Branch resolution evaluates only valid parent/sequence/fence/phase paths and compares each branch's strongest reachable authority (`terminal > executing > queued > owner maintenance`), so an ancestor-late maintenance sibling cannot dethrone an already authoritative executing/terminal descendant. A stale cancel/delete/renew/teardown can only append against its observed head and cannot overwrite or delete a newer request. Recovery releases an owner on lifecycle teardown, renews a live owner lease, and reclaims an expired owner without deleting unrelated Pending Dirty or foreground intent. A successful pull/manual-sync may clear only the exact foreground intent generation captured at its start and only after the persisted baseline covers that generation's `remoteUpdatedAt`; a newer request stays pending even when its timestamp is equal. Keep `pagehide` / `beforeunload` recovery conservative: wait for lock expiry and rerun from fresh snapshots.
