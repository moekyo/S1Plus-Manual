...
- Background, startup, foreground follow-up, and manual sync share the same mode/global execution-exclusion boundary. `runTransaction` must not resolve until baseline/writer state is persisted and covered pending/shared work is cleared; the lifecycle intentionally accepts no separate finalizer. Indicator aggregation, refresh, messages, conflict UI, retry scheduling, and manual-intent settlement run only after both locks are released. A navbar manual request has priority at the intent layer: if a valid execution is active it persists the exact direction/generation, hands off only this tab's not-yet-started automatic scheduling, and projects `Pending(push|pull)` without deleting another run's lock, heartbeat, or durable intent. At the next execution boundary it becomes `awaiting_confirmation`; confirmation owns no execution lock, re-reads current state, and only then invokes the existing manual handler. With no active execution, the request remains immediate and requires no extra confirmation. GM storage is not CAS: manual intent mutations use the same-origin exclusive `navigator.locks` decision authority and fresh revalidation, while an unavailable Web Locks primitive fails closed for delayed intent decisions.
...
- `s1pSyncLifecycleAdapter` is the single sync entry for `visibilitychange`, `pageshow`, `pagehide`, and `beforeunload`. Keep local mutation finalizers and Scheduler checkpoint work before foreground storage resync, Pending Dirty recovery, remote probing, and visible polling. Read-progress lifecycle flushes belong to its registered finalizer rather than separate DOM listeners. Keep `bind()` / `unbind()` reversible and idempotent: remove all four DOM listeners, release reference-counted GM/activity-listener leases, stop polling, and invalidate delayed callbacks on unbind. `pagehide` / `beforeunload` may hand off Scheduler Owner only; they must not take over Running Sync or delete an unsettled execution lock. Fence only the handed-off generation so the unloading tab cannot reclaim its own empty-owner notification without blocking newer generations; a canceled `beforeunload` may recover Scheduler Owner only from a later task that proves the page is still alive, visible, and still on the same lifecycle transition.
- Navbar direct pull/push (`handleForcePull` / `handleForcePush`) is an explicit manual request, not an automatic recovery path. `s1pCreateManualSyncIntentCoordinator()` is the display/request authority for these two directions: no active execution means immediate execution; an active valid mode/global lock creates durable `queued_waiting_for_execution_boundary` intent and the navbar shows the requested direction as `Pending`, never as a busy drop. After the lock boundary is free, one cross-context lease owner presents `awaiting_confirmation`; the dialog itself holds no execution lock. Request, owner, queued, executing, and terminal journal mutations enter the same-origin exclusive `navigator.locks` decision authority and fresh-revalidate the exact `requestId + generation + expectedHeadEventId`; GM read-back is not treated as CAS. Confirmation then obtains a tokenized manual execution authority before committing `executing`, and the existing force handler receives and validates that token. A changed head is stale/no-op and is never silently rebased. The canonical manual state is an append-only set of per-request/per-transition GM records enumerated with `GM_listValues`; the legacy shared key is only a change-notification mirror. The latest explicit direction uses an authority-assigned `authorityRequestSequence` (then compatibility ordering by `authorityRequestedAt`, `authoritySourceContextId`, and `authorityRequestId`), while `generation` remains exact identity evidence for the request. Irreversible executing/terminal siblings are never ordered by wall-clock time, source, event id, or static rank: the first real decision-authority commit wins operationally; multiple committed siblings without provable order fail closed. A stale cancel/delete/renew/teardown can only append against its observed head and cannot overwrite or delete a newer request. Recovery releases an owner on lifecycle teardown, renews a live owner lease, and reclaims an expired owner without deleting unrelated Pending Dirty or foreground intent. A successful pull/manual-sync may clear only the exact foreground intent generation captured at its start and only after the persisted baseline covers that generation's `remoteUpdatedAt`; a newer request stays pending even when its timestamp is equal. Keep `pagehide` / `beforeunload` recovery conservative: wait for lock expiry and rerun from fresh snapshots. If `navigator.locks` is unavailable, this implementation refuses to create or advance a delayed manual intent rather than claiming linearizable GM storage; existing durable records remain untouched for a later supported context to recover.
...
- If a sync change touches manual override, run `node tests/test-safe-sync-execution.js`; it covers execution-lock exclusion, preservation of foreign locks and durable pending work, local scheduling handoff, and stale retry cancellation.
