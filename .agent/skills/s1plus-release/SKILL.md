...
2. Update `S1Plus.js`.
   ...
   - Re-read and verify all three fields after editing.
3. Roll `CHANGELOG.md` for release.
...
4. Refresh the first-run welcome update popup in `S1Plus.js`.
   - Locate `showFirstTimeWelcomeIfNeeded`.
   - Rewrite the `bodyHtml` update highlights so they match the new `## [vX.Y.Z]` changelog content.
...
6. Validate local changes.
   - Review `git diff -- S1Plus.js CHANGELOG.md README.md`.
   - Verify versions and release date are consistent across edited content.
   - Verify `showFirstTimeWelcomeIfNeeded` highlights align with the released changelog and are written for general users.
7. Create release commit and tag.
